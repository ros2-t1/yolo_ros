...
class TrackingNode(LifecycleNode):
    ...
    def detections_cb(self, img_msg: CompressedImage, detections_msg: DetectionArray) -> None:
        ...
        cv_image = cv2.imdecode(np.frombuffer(
            img_msg.data, np.uint8), cv2.IMREAD_COLOR)
        ...
        if len(detection_list) > 0:
            ...
            det = Boxes(np.array(detection_list), (cv_image.shape[0], cv_image.shape[1]))
            ...
            if len(tracks) > 0:
                ...
                for t in tracks:
                    ...
                    tracked_box = Boxes(t[:-1], (cv_image.shape[0], cv_image.shape[1]))
